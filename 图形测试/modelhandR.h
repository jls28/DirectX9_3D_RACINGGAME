...
* マクロ定義
*******************************************************************************/
#define MAX_MODELHANDR ( 1 )                     // モデルの数

#define POSMOVE_MODELHANDR	(    5.0f )		// 移動量
...
typedef struct
{
	LPDIRECT3DTEXTURE9 pTextureModelHandR;       // テクスチャへのポインタ
	LPD3DXBUFFER pBuffMatModelHandR;             // 頂点バッファへのポインタ
	LPD3DXMESH pMeshModelHandR;                  // メッシュの情報
	DWORD nNumMatModelHandR;                     // マテリアル数
	D3DXMATRIX mtxWorldModelHandR;               // ワールドマトリックス
